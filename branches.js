// branches.js
// This file contains logic specific to branches.html, focusing on plotting branch-related data.
...
async function initializeBranchesPage() {
    ...
    if (parsedData.length > 0 && headers.length > 0) {
        ...
        // --- Other Branches-specific Event Listeners (if any) ---
        if (xAxisFilterInput) {
...
async function handleClearAllSavedGraphs() {
    ...
    if (confirmClear) {
        try {
            const savedCharts = await loadSavedCharts();
            const pageSpecificCharts = savedCharts.filter(chart => chart.chartConfig && chart.chartConfig.page === 'branches'); // Assuming you can tag charts by page
